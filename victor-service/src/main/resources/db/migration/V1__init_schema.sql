...
(
    64
) NOT NULL UNIQUE COMMENT '业务ID，用于API',
    name VARCHAR
(
    128
) COMMENT '名称',
...
(
    domain_id
) REFERENCES victor_domain
(
    id
),
    UNIQUE KEY uk_layer_domain
(
    layer_id,
    domain_id
),
...
(
    64
) NOT NULL UNIQUE COMMENT '业务ID',
    name VARCHAR
(
    128
) NOT NULL COMMENT '实验名称',
...
(
    ...
    auto_ramp_enabled TINYINT
(
    1
) DEFAULT 0 COMMENT '是否启用自动灰度推进',
...
(
    64
) NOT NULL COMMENT '实验业务ID (victor_experiment.exp_id)',
    version VARCHAR
(
    32
...
(
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uk_exp_version_bucket
(
    exp_id,
    version,
    bucket_id
...
(
    ...
    INDEX idx_exp_version
(
    exp_id,
    version
),
    INDEX idx_exp_active
(
    exp_id,
    is_active
)
...
(
    32
) NOT NULL UNIQUE COMMENT '版本号',
    etag VARCHAR
(
    16
) NOT NULL COMMENT '配置摘要',
    config_json LONGTEXT COMMENT '完整配置JSON快照',
    change_type ENUM
(
    'full',
    'incremental'
) DEFAULT 'full' COMMENT '变更类型',
...
(
    64
) NOT NULL COMMENT '实验业务ID',
    bucket_id VARCHAR
(
    64
) NOT NULL COMMENT '分桶ID',
...
(
    32
) NOT NULL DEFAULT 'pending' COMMENT '审批状态: pending/approved/rejected',
...
(
    64
) NOT NULL COMMENT '实验ID',
    report_date DATE NOT NULL COMMENT '报告日期',
    srm_passed TINYINT
(
    1
) DEFAULT 0 COMMENT 'SRM检验是否通过',
    srm_p_value DOUBLE DEFAULT NULL COMMENT 'SRM p值',
    srm_chi_square DOUBLE DEFAULT NULL COMMENT 'SRM卡方值',
    srm_message VARCHAR
(
    255
) DEFAULT NULL COMMENT 'SRM消息',
    primary_metric_name VARCHAR
(
    64
) DEFAULT 'conversion_rate' COMMENT '主指标名称',
    ...
    primary_significant TINYINT
(
    1
) DEFAULT 0 COMMENT '主指标是否显著',
    cuped_applied TINYINT
(
    1
) DEFAULT 0 COMMENT '是否应用CUPED方差缩减',
...
(
    32
) DEFAULT NULL COMMENT '建议: LAUNCH/DO_NOT_LAUNCH/CONTINUE_EXPERIMENT/INCONCLUSIVE',
    recommendation_reason VARCHAR
(
    512
) DEFAULT NULL COMMENT '建议理由',
...
(
    ...
    INDEX idx_report_date
(
    report_date
),
    UNIQUE KEY uk_exp_report_date
(
    exp_id,
    report_date
)
...
(
    64
) NOT NULL COMMENT '角色名: ADMIN/OPERATOR/VIEWER',
    description VARCHAR
(
    255
...
-- 默认域和层
INSERT
IGNORE INTO victor_domain (domain_id, name, traffic_ratio, is_exclusive) VALUES
('default', '默认域', 1.0, FALSE);

INSERT
IGNORE INTO victor_layer (layer_id, domain_id, name, salt, sort_order) VALUES
('default', 1, '默认层', 'victor_default', 0);

-- 默认角色
INSERT
IGNORE INTO rbac_role (id, name, description) VALUES
...
-- ADMIN 权限 (全部)
INSERT
IGNORE INTO rbac_role_permission (role_id, permission) VALUES
(1, 'CREATE_EXPERIMENT'),
(1, 'EDIT_EXPERIMENT'),
(1, 'DELETE_EXPERIMENT'),
(1, 'VIEW_EXPERIMENT'),
...
INSERT
IGNORE INTO victor_user (username, password_hash, email, enabled) VALUES
('admin', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'admin@gateflow.local', TRUE);

-- 管理员用户关联 ADMIN 角色
INSERT
IGNORE INTO rbac_user_role (user_id, role_id)
SELECT u.id, r.id
FROM victor_user u,
     rbac_role r
