...
public interface VariantMapper extends BaseMapper<Variant> {
     ...
     * 查询实验的所有版本（所有历史版本）
     */
    @Select("SELECT * FROM victor_variant WHERE exp_id = #{expId} ORDER BY version DESC, bucket_id ASC")
    List<Variant> selectByExpId(@Param("expId") String expId);

    /**
     * 查询实验的当前活跃版本
     */
    @Select("SELECT * FROM victor_variant WHERE exp_id = #{expId} AND is_active = TRUE ORDER BY bucket_id ASC")
    List<Variant> selectActiveVariants(@Param("expId") String expId);

    /**
     * 查询实验的指定版本
     */
    @Select("SELECT * FROM victor_variant WHERE exp_id = #{expId} AND version = #{version} ORDER BY bucket_id ASC")
    ...
    @Select("<script>SELECT * FROM victor_variant WHERE exp_id IN " +
        ...
        "AND is_active = TRUE ORDER BY exp_id, bucket_id ASC</script>")
