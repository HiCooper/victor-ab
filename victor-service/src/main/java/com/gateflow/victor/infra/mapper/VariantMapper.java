...
public interface VariantMapper extends BaseMapper<Variant> {
     ...
     * 查询实验的所有版本（所有历史版本）
     */
    @Select("SELECT id, exp_id, version, variant_key AS bucket_id, name, bucket_start, bucket_end, params, is_active, created_at FROM victor_variant WHERE exp_id = #{expId} ORDER BY version DESC, bucket_start ASC")
    List<Variant> selectByExpId(@Param("expId") String expId);

    /**
     * 查询实验的当前活跃版本
     */
    @Select("SELECT id, exp_id, version, variant_key AS bucket_id, name, bucket_start, bucket_end, params, is_active, created_at FROM victor_variant WHERE exp_id = #{expId} AND is_active = TRUE ORDER BY bucket_start ASC")
    List<Variant> selectActiveVariants(@Param("expId") String expId);

    /**
     * 查询实验的指定版本
     */
    @Select("SELECT id, exp_id, version, variant_key AS bucket_id, name, bucket_start, bucket_end, params, is_active, created_at FROM victor_variant WHERE exp_id = #{expId} AND version = #{version} ORDER BY bucket_start ASC")
     ...
     * 批量查询多个实验的活跃变体
     */
    @Select("<script>SELECT id, exp_id, version, variant_key AS bucket_id, name, bucket_start, bucket_end, params, is_active, created_at FROM victor_variant WHERE exp_id IN " +
