...
UPDATE victor_experiment SET status = 'pending_approval' WHERE status IN ('approved', 'review');
UPDATE victor_experiment SET status = 'draft'              WHERE status = 'rejected';
UPDATE victor_experiment SET status = 'running'            WHERE status = 'ramp';
UPDATE victor_experiment SET status = 'stopped'            WHERE status IN ('paused', 'analyzing', 'decision');
